...
(define (click-piece p game)
  (Checkers (remove-p (Checkers-pieces game) p) (Checkers-turn game) p))
...
(define (valid-move? game loc)
  ;;(if (symbol? (get-piece-or-none loc game)) #f
      (match* (game loc)
        [((Checkers pieces turn (Piece color (Loc cur-r cur-c))) (Loc r c))
         (and (symbol=? turn color)
              (local
                {
                 (: turn-side : Integer)
                 (define turn-side (if (symbol=? color 'red) 1 -1))
                 
                 (: direction : Integer)
                 (define direction (if (>= c cur-c) 1 -1))
                 
                 (: one-way : (U 'none Piece))
                 (define one-way (get-piece-or-none (Loc (+ cur-r turn-side) (+ cur-c direction)) game))

                 (: two-way : (U 'none Piece))
                 (define two-way (get-piece-or-none (Loc (+ cur-r (* 2 turn-side)) (+ cur-c (* 2 direction))) game))

                 (: one-valid : Loc)
                 (define one-valid (Loc (+ cur-r turn-side) (+ cur-c direction)))

                 (: two-valid : Loc)
                 (define two-valid (Loc (+ cur-r (* 2 turn-side)) (+ cur-c (* 2 direction))))
                 }
                (or (and (loc=? (Loc r c) one-valid) (symbol? one-way))
                    (and (loc=? (Loc r c) two-valid) (symbol? two-way)))))]))

(: make-turn : Checkers -> Checkers)
(define (make-turn game)
  (match game
    [(Checkers _ 'red _) (Checkers (Checkers-pieces game) 'black (Checkers-clicked-piece game))]
    [(Checkers _ 'black _) (Checkers (Checkers-pieces game) 'red (Checkers-clicked-piece game))]))
...
(define (click-board game x y event)
  (match event
    ["button-up" (match (Checkers-clicked-piece game)
                   [(Piece _ _) (if (valid-move? game (xy->loc x y)) (place-piece (xy->loc x y) (make-turn game)) game)]
...
(define (count-color lst c)
  (match (first lst)
    [(Piece c _) (+ 1 (count-color (rest lst) c))]
    [_ 0]))
...
(valid-move? (Checkers starting-pieces 'red (Piece 'red (Loc 3 2))) (Loc 4 3))
